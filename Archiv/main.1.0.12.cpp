...
// FIRMWARE VERSION
// =======================
#define FW_VERSION "1.0.12"  // WebUI Modifiziert - Rotlicht - Dämmerung Fix
...
#define CLOSE_FORECAST_STABLE_MS 180000  // 

// ===== DEBUG / MANUELLE WS2812 TEST LED =====
...
const float MIN_NEG_LUX_RATE = 0.25;  // Schließen
...
bool actionLock = false; // Sperre für Aktionen (z.B. während OTA oder bei kritischen Fehlern)
...
void updateMotor() {
  ...
  if (millis() >= motorUntil) {
    ...
    if (motorState == MOTOR_OPENING) {
      ...
      doorPhase = PHASE_IDLE;
      saveDoorState();
      lightBelowSince = 0;   // für Prognose-Logik
...
void ws2812RedOn() {
  ...
  stallLight.clear();            // LEDs komplett zurücksetzen
  stallLight.setBrightness(255); // maximale Helligkeit
    ...
    now.hour() < CLOSE_WINDOW_START_H)
{
  ...
  if (automatikErlaubt && luxValid && openMode == "light" && !doorOpen && !learningActive && !nightLock) {
    ...
    // Öffnen NUR wenn Schwelle erreicht
if (!actionLock && doorOpen == false && lux >= openLightThreshold) {
      openInterruptionSince = 0;
...
// ===== Vor-Licht Prognose (stabilisiert) =====
...
if (forecastNow != closeForecastCondition) {
    closeForecastCondition = forecastNow;
    closeForecastStableSince = millis();
}

if (closeForecastCondition &&
    millis() - closeForecastStableSince > CLOSE_FORECAST_STABLE_MS) {
    ...
    if (doorOpen && minutesToThresh <= lampPreClose && !preLightCloseDone){
        ...
        addLogWithLux(
            "Locklicht vor Schließen gestartet (stabile Prognose)",
            lux);
    ...
    // ------------------------------------------------------------
    if (scheduledCloseAt != 0 && millis() >= scheduledCloseAt && !preLightCloseDone){
      if (!manualLightActive) {
        lightState = LIGHT_PRE_CLOSE;
...
if (doorOpen && motorState == MOTOR_STOPPED && lux <= closeLightThreshold) {
...
void handleStatus() {
  ...
  doc["ws2812red"] = ws2812RedActive ? "An" : "Aus";
...
async function toggleWS2812(){

  const b = document.getElementById('ws2812Btn');

  await debounceAction(b, async () => {

    await fetch('/ws2812red', { cache:'no-store' });
...
<div class="status-row">
  <span class="label">WS2812 Test:</span>
  <span id="ws2812red" class="badge closed">---</span>
  </div>
  <div class="status-row">
...
<button type="button" id="ws2812Btn"
  class="light-btn off"
  onclick="toggleWS2812()"
  title="WS2812 Test">
  🔴
</button>
    </div>
...
async function update(){
  ...
  try{
    ...
    // WS2812 Status anzeigen
const ws = document.getElementById("ws2812red");

if (ws && d.ws2812red) {

  ws.innerText = d.ws2812red;

  ws.className = "badge " + (d.ws2812red === "An" ? "open" : "closed");

}
  const wsBtn = document.getElementById("ws2812Btn");

if (wsBtn) {

  wsBtn.classList.toggle("on", d.ws2812red === "An");
